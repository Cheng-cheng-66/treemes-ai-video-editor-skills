...
def _parse_geometry(payload: dict[str, Any]) -> SourceGeometry:
    ...
    try:
        ...
        raise ValueError("ffprobe returned invalid video geometry") from error
    ...
    if abs(ratio - 1.0) < 0.01:
        raise ValueError(
            "source is square or aspect ratio is ambiguous; set output_aspect_ratio explicitly"
        )
...
def probe_source_geometry(source: Path, ffprobe: str = "ffprobe") -> SourceGeometry:
    result = run_command(
        [
            ffprobe,
            "-v",
    ...
    if result.returncode != 0:
        raise RuntimeError(f"unable to inspect source aspect ratio: {result.stderr.strip()}")
    try:
        return _parse_geometry(json.loads(result.stdout))
...
def resolve_output_geometry(
    ...
    if requested_aspect_ratio == "source":
        ...
        raise ValueError("output_aspect_ratio must be source, 16:9 or 9:16")
